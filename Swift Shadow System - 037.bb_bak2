...
Global Shadow_Nudge# = 0.03


;Include "prom.bb"
;Global layer=SS_CreateLayer()
...
; -------------------------------------------------------------------------------------------------------------------
Function Cast_Shadow(Caster, Resolution=64, Translucent=0)
...
; source, and prevent shadows from casting onto unwanted surfaces.
; -------------------------------------------------------------------------------------------------------------------
Function Cast_Static_Shadow(Caster, Resolution=512)
		...
		For ThisCaster.Static_Shadow_Caster = Each Static_Shadow_Caster
			...
			If Light_To_Caster_Distance# <= ThisLight\Range#
				...
				; Render the shadow for this caster from the viewpoint of this light source.
					...
					; Place the copy of the shadow caster at the same location as the real entity.
					PositionEntity Shadow_Caster, Caster_X#, Caster_Y#, Caster_Z#


					; Rotate the copy of the shadow caster to the same orientation as the real entity.
					RotateEntity Shadow_Caster, Caster_Pitch#, Caster_Yaw#, Caster_Roll#
					...
					PositionEntity Shadow_Caster, 0, Shadow_Cam_Position# + Shadow_Caster_Offset#, 0
					...
					If ThisReceiver\Entity <> ThisCaster\Entity 
						...
						If Receiver_In_Region
							...
							EntityTexture ThisShadow\Mesh, ThisTexture\Texture
                                    ;  EntityAlpha ThisShadow\Mesh,.1

                                   ; enR = SS_CreateEntity(ThisShadow\Mesh,layer)
                             ; ss_positionentity   enR,EntityX(ThisShadow\Mesh),EntityY(ThisShadow\Mesh),EntityZ(ThisShadow\Mesh)
                                   ; EntityBlend  layer ,2 
                                   ; EntityAlpha layer,.1
                                    ; FreeEntity ThisShadow\Mesh
                              TranslateEntity ThisShadow\Mesh,0,2,0
                                      ; ScaleEntity ThisShadow\Mesh,1,1,1
