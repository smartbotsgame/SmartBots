...
Select True
Case code=1 ; pop_
id%=PeekInt(s\s, k*4) ; определ€ем индекс переменной
glTop=glTop-1
; glTop=0
buf%=PeekInt(glStack, glTop*4) ; извлекаем из стэка биты
PokeInt s\var, (id-1)*4, PeekInt(glStack, glTop*4) ; заносим биты в переменную
k=k+1
Case code=2 ; push var
id%=PeekInt(s\s, k*4) ; определ€ем индекс переменной
buf%=PeekInt(s\var, (id-1)*4) ; извлекаем биты из переменной
PokeInt glStack, glTop*4, PeekInt(s\var, (id-1)*4) ; заносим в стэк
glTop=glTop+1
k=k+1
